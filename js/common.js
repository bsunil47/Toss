$(document).ready(function () {  //alert("I am in common js");
    ...
    ({
        ...
        'sAjaxSource': base_url + 'Admin/users/vendorslist',
        "bPaginate": true,
        "sPaginationType": "full_numbers",
        "iDisplayLength": 100,
        "aLengthMenu": [[100, 200, 500], [100, 200, 500]],
        ...
        'fnServerData': function (sSource, aoData, fnCallback) { //alert("I am in commonjs");
            ...
            ({
                ...
                'success':function(json){
                    $('.itotaldisrecords').html('('+json.iTotalDisplayRecords+')');
                    fnCallback(json);
                }
            });
    ...
    $('#contentsubmit').on('click', function () {
        ...
        if (fromdate != '' && todate != '') {
			...
			//var fromdate_ts = new Date(fromdate.split("-").reverse().join("-")).getTime();
			//alert(fromdate_ts);
            var todate_ts = new Date(todate).getTime();
		   //var todate_ts = new Date(todate.split("-").reverse().join("-")).getTime();
			//alert(todate_ts);
        ...
        contentTable.fnFilter(catid, 2, true);
        contentTable.fnFilter(lang, 3, true);//Exact value, column, reg
    });
    ...
    $('#catid').on('change', function () {
        ...
        contentTable.fnFilter(fromdate, 0, true); //Exact value, column, reg
        contentTable.fnFilter(todate, 1, true);
        contentTable.fnFilter(selectedValue, 2, true);
        contentTable.fnFilter(lang, 3, true);//Exact value, column, reg
    });
    $('#language').on('change', function () {
        ...
        contentTable.fnFilter(catid, 2, true);//Exact value, column, reg
         contentTable.fnFilter(selectedValue, 3, true);
    });
    ...
    $('#delcontentsubmit').on('click', function () {
        ...
        contentmTable.fnFilter(lan, 3, true);
    });
    ...
    $('#lang').on('change', function () {
        ...
        contentmTable.fnFilter(fromdate, 0, true); //Exact value, column, reg
        contentmTable.fnFilter(todate, 1, true);
        contentmTable.fnFilter(catids, 2, true); //Exact value, column, reg
        contentmTable.fnFilter(selectedValue, 3, true);
    });
    ...
    $('#catidval').on('change', function () {
        ...
        wTable.fnFilter(selectedValue, 0, true); //Exact value, column, reg
         wTable.fnFilter(lan, 1, true);
    });
    $('#langs').on('change', function () {
        var cati = $('#catidval').val();
        var selectedValue = $(this).val();
        
        //alert(selectedValue);
        wTable.fnFilter(cati, 0, true); //Exact value, column, reg
         wTable.fnFilter(selectedValue, 1, true);
    });
