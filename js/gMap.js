...
var searchBox = new google.maps.places.Autocomplete(
    /** @type {HTMLInputElement} */(document.getElementById('tossvenueform-toss_ven_location'))
    );
...
function showPosition(position) {
    ...
    if ($('#tossvenueform-toss_ven_location').length > 0  || $('.common-route-location').length > 0) {
        ...
        latlng = new google.maps.LatLng($('#tossvenueform-toss_ven_lat').val(), $('#tossvenueform-toss_ven_lng').val());
...
function codeLatLng(lat, Lng) {
    ...
    geocoder.geocode({'latLng': latlng}, function (results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
            if (results[1]) {
                map.setZoom(17);
                if ($('#tossvenueform-toss_ven_location').length > 0) {
                    $('#tossvenueform-toss_ven_location').val(results[1].formatted_address);
                    $('#tossvenueform-toss_ven_lat').val(lat);
                    $('#tossvenueform-toss_ven_lng').val(Lng);
                }
                if ($('#tossvenueform-toss_ven_location').length > 0) {
                    $('#tossvenueform-toss_ven_location').val(results[1].formatted_address);
                    $('#tossvenueform-toss_ven_lat').val(lat);
