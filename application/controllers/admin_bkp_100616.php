...
class Admin extends My_Controller
{
    ...
    {
        $login_array = ['admin/dashboard', 'admin/dashboard', 'vendors/dashboard', 'venue/dashboard'];
    ...
    public function editvendordetails()
    {
        ...
        if (isset($_POST['vendorsubmit'])) {
            if ($this->form_validation->run('edit_vendor') == FALSE) {
                ...
                $data = array(
                    "name" => ucfirst($this->input->post('uname')),
                ...
                $vendordata = array(
                    "company_name" => ucfirst($this->input->post('c_name')),
                ...
                $bankdata = array(
                    "beneficiary_name" => ucfirst($this->input->post('benf_name')),
    ...
    public function editvenuedetails()
    {
        if (isset($_POST['vendorsubmit'])) {
            ...
            $venuedata = array(
                "venue_display_name" => ucfirst($this->input->post('v_disp_name')),
            ...
            if (!empty($_FILES)) {
                foreach ($_FILES as $key => $value) {
                    ...
                    if (!in_array($imagefiletype, $allowed_extensions)) {
                        $this->session->set_flashdata("image_error", "Problem with Uploading Images");
                        redirect(base_url("admin/editvenuedetails"));
                    } else {
                        if ($key == 'profile_pic') {
                            $targetuserdir = "images/profiles/$vendor_id";
                            $targetfileuser = $targetuserdir . "_" . "user.jpg";
    ...
    public function addvendor()
    {
        ...
        //echo "<pre>";print_r($subsubcategoryid);exit;
        $this->common_model->initialise("categories");
        $data['getallcategories'] = $this->common_model->get_records(0, "*", '');
        $this->common_model->initialise("sub_categories");
        $data['getallsubcats'] = $this->common_model->get_records(0, "*", '');
        ...
        if (isset($_POST['venuesubmit'])) {
            if ($this->form_validation->run('add_user') == FALSE) {
                ...
                if ($this->input->post('tandc') == 1) {
                    $user_pic = $_FILES;
                    $user_array = array('uname' => "name", "u_email" => 'email', "u_passw" => 'assword', "gender" => 'gender', "u_phone" => 'phone');
                    ...
                    foreach ($params as $key => $value) {
                        if (array_key_exists($key, $user_array)) {
                            $data[$user_array[$key]] = mysqli_real_escape_string($this->db->conn_id, $value);
                        }
                        if (array_key_exists($key, $vendor_array)) {
                            $vendordata[$vendor_array[$key]] = mysqli_real_escape_string($this->db->conn_id, $value);
                        }
                        if (array_key_exists($key, $bank_array)) {
                            $bankdata[$bank_array[$key]] = mysqli_real_escape_string($this->db->conn_id, $value);
                        }
                        if (array_key_exists($key, $venue_array)) {
                            $venuedetails[$venue_array[$key]] = mysqli_real_escape_string($this->db->conn_id, $value);
                        }
                    }
                ...
                if ($insertvenuedetails) {
                    ...
                    if ($insertcategory) {
                        for ($j = 0; $j < count($subcategoryid); $j++) {
                            $subcategorydata = array(
                                "venue_id" => $venue_id,
                                "sub_category_id" => $subcategoryid[$j],
                                "status" => 1
                            );
                        ...
                        if ($insertsubcategory) {
                            for ($k = 0; $k < count($subsubcategoryid); $k++) {
                                $subsubcategorydata = array(
                                    "venue_id" => $venue_id,
                                    "sub_sub_category_id" => $subsubcategoryid[$k],
                                    "status" => 1
                                );
                            ...
                            if ($insertsubsubcategory) {
                                ...
                                foreach ($days as $key => $value) {
                                    ...
                                    $workingdata = array(
                                        ...
                                        "sub_sub_category_id" => $subsubcatid,
                                        "day_id" => $value,
    ...
    public function editvenueinfo($venueid, $rtype, $rid)
    {
        ...
        $data['getallsubsubcategories'] = $this->common_model->get_records(0, "*", '');
        $this->common_model->initialise("facilities");
        $data['getfacilites'] = $this->common_model->get_records(0, "*", '');
        ...
        $this->load->view("edit_venue", $data);
    ...
    public function viewvenueusersinfo($venueid)
    {
        ...
        $this->load->view("manage_venueusers", $data);
        //$this->viewvenueusersinfodetails($venueid,$url);
    ...
    {
        //echo $this->input->post('gender');exit;
    ...
    public function addcategory()
    {
        if (isset($_POST['catsubmit'])) {
            ...
            if ($this->form_validation->run('add_cat') == FALSE) {
                ...
                $data = array(
                    "category_name" => ucfirst($this->input->post('cat_name')),
                    "status" => 1
                );
                $insertcategory = $this->db->insert("tbl_categories", $data);
                $categoryid = $this->db->insert_id();
                $target_dir = "images/categories";
                $targetfile = $target_dir . "/" . "{$categoryid}_category" . ".jpg";
                $categorydata['category_image'] = "{$categoryid}_category" . ".jpg";
                $this->updatedetails($categorydata, "categories", $categoryid);
    ...
    public function editcategorydetails()
    {
        ...
        $target_dir = "images/categories";
        $targetfile = $target_dir . "/" . "{$category_id}_category" . ".jpg";
        $facilitydata['category_image'] = "{$category_id}_category" . ".jpg";
    ...
    {
        $this->common_model->initialise('categories');
        $data['getallcategories'] = $this->common_model->get_records(0, '*', array('status' => 1));
            ...
            if ($this->form_validation->run("add_subcat") == FALSE) {
                ...
                if ($_POST['category_id']) {
                    ...
                    for ($i = 0; $i < count($cat); $i++) {
                        $data = array(
                            "category_id" => $cat[$i],
                            "sub_category_name" => ucfirst($this->input->post('subcat_name')),
                            "status" => 1
                        );
                        $insertsubcat = $this->db->insert("tbl_sub_categories", $data);
                        $subcatid = $this->db->insert_id();
                        $target_dir = "images/categories";
                        $targetfile = $target_dir . "/" . "{$cat[$i]}_{$subcatid}_sub_category" . ".jpg";
                        $subcategorydata['sub_category_image'] = "{$cat[$i]}_{$subcatid}_sub_category" . ".jpg";
                        $this->updatedetails($subcategorydata, "sub_categories", $subcatid);
            ...
            if ($this->form_validation->run("add_subcat") == FALSE) {
                ...
                if ($_POST['category_id']) {
                    ...
                    for ($i = 0; $i < count($cat); $i++) {
                        ...
                        $target_dir = "images/categories";
                        $targetfile = $target_dir . "/" . "{$cat[$i]}_{$subcat_id}_sub_category" . ".jpg";
                        $subcategorydata['sub_category_image'] = "{$cat[$i]}_{$subcat_id}_sub_category" . ".jpg";
    ...
    public function managesubsubcategories()
    {
        ...
        $this->load->view("manage_subsubcategory", $data);
    }
    ...
    public function addsubsubcategory()
    {
        ...
        $this->common_model->initialise("categories");
        $data['getallcategories'] = $this->common_model->get_records(0, "*", '');
        //echo "<pre>";print_r($data['getallcategories']);exit;
        $this->common_model->initialise("sub_categories");
        $data['getallsubcategories'] = $this->common_model->get_records(0, "*", '');
        ...
        if (isset($_POST['subsubcatsubmit'])) {
            ...
            if ($this->form_validation->run("add_subsubcat") == FALSE) {
                ...
                if ($_POST['category_id']) {
                    ...
                    //echo "<pre>";print_r($cats);echo "before loop cats";
                    //for($i=0;$i<count($cat);$i++){
                    foreach ($cats as $key => $value) {

                        $data = array(
                            "category_id" => $value,
                            "sub_category_id" => $subcats[$value],
                            "sub_sub_category_name" => ucfirst($this->input->post('subsubcat_name')),
                            "status" => 1
                        );
                        $insertsubsubcat = $this->db->insert("tbl_sub_sub_categories", $data);
                        $subsubcatid = $this->db->insert_id();
                        $target_dir = "images/categories";
                        $targetfile = $target_dir . "/" . "{$value}_{$subcats[$value]}_{$subsubcatid}_sub_sub_category" . ".jpg";
                        $subsubcategorydata['sub_sub_category_image'] = "{$value}_{$subcats[$value]}_{$subsubcatid}_sub_sub_category" . ".jpg";
                        $this->updatedetails($subsubcategorydata, "sub_sub_categories", $subsubcatid);
    ...
    public function editsubsubcategory($subsubcategory_id)
    {
        ...
        //echo "<pre>";print_r($data['editsubsubcatinfo']);exit;
        ...
        if (isset($_POST['subsubcatsubmit'])) {
            ...
            if ($this->form_validation->run("edit_subsubcat") == FALSE) {
                ...
                if ($_POST['category_id']) {
                    ...
                    $target_dir = "images/categories";
                    $targetfile = $target_dir . "/" . "{$cat}_{$subcat_id}_{$subsubcat_id}_sub_sub_category" . ".jpg";
                    $subsubcategorydata['sub_sub_category_image'] = "{$cat}_{$subcat_id}_{$subsubcat_id}_sub_sub_category" . ".jpg";
    ...
    public function managepricing($venueid)
    {
        ...
        $this->common_model->initialise("venues");
        /*$this->common_model->join_tables=array("prices as P","vendor as VD");
	$this->common_model->join_on=array("P.venue_id = V.venue_id","V.vendor_id = VD.vendor_id");
    ...
    public function viewprice($id, $venueid)
    {
        ...
        $this->common_model->join_tables = array('venues as V', 'membership_types as M', 'categories as C', 'sub_categories as S', 'vendor as VD', 'facilities as F', 'sub_sub_categories as SC', 'venues as VE');
        $this->common_model->join_on = array("P.venue_id = V.venue_id", "P.base_type_id = M.membership_type_id", "P.base_type_id = C.category_id", "P.base_type_id = S.sub_category_id", "V.vendor_id = VD.vendor_id", "P.base_type_id = F.facility_id", "P.base_type_id = SC.sub_sub_category_id", "P.base_type_id = VE.venue_id");
        $this->common_model->left_join = array('left', 'left', 'left', 'left', 'left', 'left', 'left', 'left');
        $select = "P.*,VD.company_name,V.venue_display_name,M.membership_name,C.category_name,S.sub_category_name,F.facility_name,SC.sub_sub_category_name,VE.venue_display_name as vname";
        $where = "P.price_id = " . $id;
        $data['getdetails'] = $this->common_model->get_record_single($where, $select);
        $this->load->view("viewprice", $data);
    }
 ...
 $this->common_model->initialise("facilities as F");
	...
	$data['getallfacilities']=$this->common_model->get_records(0,"*",$where,$col = 0, $order = 'desc',$groupby);
 ...
 */
        ...
        $this->load->view("addprice", $data);
    }

    public function editprice($id, $venueid)
    {
        $data = array();
        $data['priceid'] = $id;
        $data['venueid'] = $venueid;
        ...
        $this->common_model->initialise('venues');
        $data['slotvenues'] = $this->common_model->get_records(0, '*', '');
        $venueid = $this->input->post('venue_id');
        /*$this->common_model->initialise("facilities as F");
	$this->common_model->join_tables = array("venue_facilities as VF","venue_users as VU");
	$this->common_model->join_on = array("F.facility_id = VF.facility_id","VF.venue_id = VU.venue_id");
	...
	$data['facilities']=$this->common_model->get_records(0,"F.*,F.status as facilitystatus,F.facility_id as facid",$where,$col = 0, $order = 'desc',$groupby);*/
        ...
        $data['vendordetails'] = $this->common_model->get_records(0, '*', '');
        $this->common_model->initialise('prices as P');
        $this->common_model->join_tables = array('venues as V', 'membership_types as M', 'categories as C', 'sub_categories as S', 'facilities as F', 'vendor as VD', 'sub_sub_categories as SC', 'venues as VE');
        $this->common_model->join_on = array("P.venue_id = V.venue_id", "P.base_type_id = M.membership_type_id", "P.base_type_id = C.category_id", "P.base_type_id = S.sub_category_id", "P.base_type_id = F.facility_id", "V.vendor_id = VD.vendor_id", "P.base_type_id = SC.sub_sub_category_id", "P.base_type_id = VE.venue_id");
        $this->common_model->left_join = array('left', 'left', 'left', 'left', 'left', 'left', 'left', 'left');
        $select = "P.*,VD.vendor_id,VD.company_name,V.venue_id,V.venue_display_name,M.membership_type_id,M.membership_name,C.category_id,C.category_name,S.sub_category_id,S.sub_category_name,F.facility_id,F.facility_name,SC.sub_sub_category_name,VE.venue_display_name as vname";
        $where = "P.price_id = " . $id;
        $data['getdetails'] = $this->common_model->get_record_single($where, $select);
        $this->common_model->initialise('venues');
        ...
        $this->load->view("editpricing", $data);
    }
    ...
    public function manageaddons($venueid)
    {
        ...
        $where = "venue_id = " . $venueid;
        $data['getdetails'] = $this->common_model->get_record_single($where, $select);
        $this->load->view('manageaddon', $data);
    }
    ...
    public function addaddon($venueid)
    {
        ...
        //echo "<pre>"; print_r($data['subsubcategories']);exit;
        $data['facilities'] = $this->getvfacilities();
        //$subsubcatid=$this->input->post('sub_sub_catid');
        $this->load->library("form_validation");

        /*$this->form_validation->set_rules("addon","Addon","required|trim");
	$this->form_validation->set_rules("addon_name","Addon Name","required|trim");*/
        ...
        if (isset($_POST['addonsubmit'])) {
            ...
            if ($this->form_validation->run("add_addon") == FALSE) {
                ...
                $addondata = array('venue_id' => $_POST['venue_id'], 'base_type_id' => $_POST['type'], 'base_type' => $_POST['ptype'], 'amount' => $_POST['amount'], 'status' => 1, 'created_by' => $this->session->userdata['user_id']);
                //$adddonsubsubcatdata=array('venue_id' => $_POST['venue_id'],'base_type_id'=>$_POST['subcat_id'],'base_type'=>3,'amount'=>$_POST['amount'],'status'=>1,'created_by'=>$this->session->userdata['user_id']);
                ...
                if ($_POST['addon'] == 'other') {
                    ...
                    $addondata['addon_name'] = ucfirst($facility->facility_name);
                }
                $this->common_model->initialise('addon');
 ...
 * **/
    ...
    public function managevenuefacilities($venueid)
    {
        ...
        $where = "V.venue_id = '$venueid'";
        $data['getdetails'] = $this->common_model->get_record_single($where, $select);
        $this->load->view("manage_venue_facilities", $data);

    }

    public function managevenuefacilities1($venueid)
    {
        $data = array();
        $data['venueid'] = $venueid;
        $this->common_model->initialise("venue_facilities  as VF");
        $this->common_model->join_tables = array("venues as V", "facilities as F", "vendor as VD");
        $this->common_model->join_on = array("VF.venue_id = V.venue_id", "VF.facility_id = F.facility_id", "V.vendor_id = VD.vendor_id");
        $data['getallfacilities'] = $this->common_model->get_records(0, "VF.*,F.facility_name,V.venue_display_name", "VF.venue_id = '" . $venueid . "'");
        //$data['getallfacilities']=$this->common_model->get_record_single("VF.venue_id='$venueid'","VF.*,F.facility_name,V.venue_display_name");
        $this->load->view("manage_venue_facilities", $data);
    ...
    public function addvenuefacility($venueid)
    {
        ...
        $this->common_model->join_tables = array("venues as V", "facilities as F", "vendor as VD", "venue_category as VC");
        $this->common_model->join_on = array("VF.venue_id = V.venue_id", "VF.facility_id = F.facility_id", "V.vendor_id = VD.vendor_id", "F.base_type_id=VC.category_id");
        $data['facilities'] = $this->common_model->get_records(0, "VF.*,F.facility_name,V.venue_display_name", "VF.venue_id = '" . $venueid . "'", '', '', "F.facility_id");
        //$data['facilities']=$this->common_model->get_records(0,"*","");
        ...
        $this->load->view('add_venue_facilities', $data);
    }

    public function addvenuefacility1($venueid)
    {
        ...
        if (isset($_POST['faclsubmit'])) {
            ...
            if ($this->form_validation->run("add_facility") == FALSE) {
                ...
                $this->common_model->initialise('venue_facilities');
                $facarr = $_POST['facility'];
                $fid = $this->categories_sort('facility_id', $facarr, $venueid);
                if (isset($fid)) {
                    $this->session->set_flashdata("facility_success", "You Have Added Facility Successfully");
                    redirect(base_url("admin/managevenuefacilities/$venueid"));
                }
    ...
    public function viewfacility($fid, $vid)
    {
        ...
        $this->load->view("view_venue_facility", $data);
    }

    public function editfacilityvenueinfo1($fid, $vid)
    {
        $this->common_model->initialise('venues as V');
        $this->common_model->join_tables = array("vendor as VD");
        $this->common_model->join_on = array("V.vendor_id = VD.vendor_id");
    ...
    public function managecoins()
    {
        ...
        $select = "*";
        $data['getallcoins'] = $this->common_model->get_records(0, $select, '');
        $this->load->view("manage_coins", $data);
    }
    ...
    public function editcoinsinfo($coinid)
    {
        ...
        $data['editcoin'] = $this->common_model->get_record_single($where, $select);
        $this->load->view("edit_coin", $data);
    ...
    public function editcoindetails()
    {
        ...
        if (isset($_POST['coinsubmit'])) {
            ...
            if ($this->form_validation->run("edit_coin") == FALSE) {
                ...
                $where = "coin_id = " . $coinid;
                $coinupdatedata = $this->common_model->update($where);
    ...
    public function addfacility()
    {
        if (isset($_POST['faclsubmit'])) {
            ...
            if ($this->form_validation->run("add_subcat") == FALSE) {
                ...
                $facilitydata = array(
                    "base_type" => $this->input->post('ftype'),
                    "base_type_id" => $this->input->post('type'),
                    "facility_name" => addslashes(ucfirst($this->input->post('faclty_name'))),
                    "status" => 1
                );
                $insertaddfaclty = $this->db->insert("tbl_facilities", $facilitydata);
    ...
    public function viewfacilityinfo($facility_id)
    {
        ...
        $this->common_model->initialise("facilities as F");
        $this->common_model->join_tables = array("categories as C", "sub_categories as SC");
        $this->common_model->join_on = array("F.base_type_id=C.category_id", "F.base_type_id=SC.sub_category_id");
        $this->common_model->left_join = array("left", "left");
        $select = "F.*,C.category_name,SC.sub_category_name";
        $where = "F.facility_id = " . $facility_id;
        $data['viewfacilities'] = $this->common_model->get_record_single($where, $select);
        $this->load->view("view_facility", $data);
    }
    ...
    public function editfacilityinfo($facility_id)
    {
        ...
        $this->common_model->initialise('sub_categories');
        $data['subcategories'] = $this->common_model->get_records(0, '*', '');
        $this->load->view("edit_facility", $data);
    }
    ...
    public function editfacilitydetails()
    {
        ...
        $facilitydata = array(
            "base_type" => $this->input->post('ftype'),
            "base_type_id" => $this->input->post('type'),
            "facility_name" => ucfirst($this->input->post('faclty_name'))
        );
        $this->common_model->initialise("facilities");
    ...
    {
        $this->db->where("facility_id", $deleteid);
        $this->db->delete("tbl_facilities");
        redirect(base_url("admin/managefacilities"));
    }
...
private function getallcategoriesdata($table){
    $this->common_model->initialise($table);
    return $this->common_model->get_records(0,"*","");
}
    ...
    public function vendorupload()
    {
        ...
        $this->load->view("vendorupload", $data);
    }
    ...
    public function manageimages($venueid, $vendorid)
    {
        ...
        $this->load->view("venue_images", $data);
    }
