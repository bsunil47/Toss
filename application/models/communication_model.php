...
class Communication_model extends CI_Model
{
    ...
    private $from_email = "tossadmin@gmail.com";
    ...
    public function send_mail($email)
    {
        ...
        $message .= '<div>Toss App</div>';
        $message .= '<div> Email : ' . $this->from_email . '</div>';
        $message .= '</body></html>';

        $this->email->from($this->from_email, 'TOSS');
        $this->email->to($email);
...
// Additional headers
        ...
        $headers .= "From: Toss <{$this->from_email}>" . "\r\n";
...
// Mail it
    ...
    function send_recover_code($data) {
     ...
     //   ($path == 'frontend') ? $pathi = 'Content Owner' : $pathi = $path;
        ...
        $message = "<html><head></head><body><div><img src='{$url}img/logo-icon.png' /></div>"
            . "<div>&nbsp;</div>"
            . "<div>Please click on the following link to Change the Password</div>"
            . "<div>&nbsp;</div>"
            . "<div><a href='{$url}frontend/forgot_changepassword/{$message}'>" . "Change password link" .
            '</a></div></body></html>';
        // To send HTML mail, the Content-type header must be set
        //echo $message; exit;
        $headers = 'MIME-Version: 1.0' . "\r\n";
