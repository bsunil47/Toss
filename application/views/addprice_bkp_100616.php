					...
					<?php }?>
						...
						<div class="form-group">
                            ...
                            <div class="col-sm-10">
                               <select name="ptype" id="ptype" onchange="getval(this.value)" class="form-control m-b-10">
                                    ...
                                    <option value="5">Sub Subcategory</option>
									<option value="6">Slot</option>
                                </select>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
								...
								<!--<select id="type" class="select2-multiple form-control m-b-10" name="type[]">-->
...
function getval(v){
	//alert(v);
        if(v=='1'){
        var url='<?php echo base_url()?>admin/getmembership';
	...
	var url='<?php echo base_url()?>admin/getfacilities';
	...
	var first="Select Subsubcategory";
	jQuery.ajax({
		type:'POST',
		url :url,
		data:{'id':v},
		...
		success: function(data){
			jQuery('#type').html(data);
			jQuery("#type").attr('multiple','multiple');
			jQuery('#type').append('<option value="">'+first+'</option>');
			var placeholder = "Select One";
				jQuery('select#type').select2({
						placeholder: placeholder
			...
			for(i in data){
				data[i].sub_sub_category_name=data[i].sub_sub_category_name.replace(/\\/g, '');
				jQuery('#type').append('<option value="'+data[i].sub_sub_category_id+'">' + data[i].sub_sub_category_name+'</option>');
			}
		}
	});	
	}else if(v=='6'){
       var url='<?php echo base_url()?>admin/getslots';
    var first="Select Venue";
    jQuery.ajax({
		type:'POST',
		url:url,
		data:{'id': v },
		...
		success: function(data){
			//jQuery(".subcategory option[value='x']").remove();
                        if(data==''){
        alert("There is No Slots For The Current Venue First add slots");
           }
			jQuery('#type').html(data);
			jQuery('#type').removeAttr('multiple');
			jQuery('#type').append('<option value="">' +  first + '</option>');
			...
			for(i in data){
                            data[i].venue_display_name=data[i].venue_display_name.replace(/\\/g, '');
				jQuery('#type').append('<option  value="'+ data[i].venue_id + '">' + data[i].venue_display_name +'</option>');
			}
